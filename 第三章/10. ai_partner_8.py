...
def load_sessions():
    ...
    session_list.sort(reverse=True) # 排序, 降序排序, reverse默认为False, 升序排序
    return session_list
...
# Logo
st.logo(image="resources/logo.png")
...
with st.sidebar:
    ...
    for session in session_list:
        col1, col2 = st.columns([4, 1]) # 解包操作
        with col1:
            # 加载会话信息
            # 三元运算符: 如果条件为真, 则返回第一个参数, 否则返回第二个参数 --> 语法: 值1 if 条件 else 值2
...
if prompt: # 字符串会自动转换为布尔值, 如果字符串非空, 则为True; 否则为False
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            *st.session_state.messages # 列表中的解包, 别忘记了!!!
        ],
        stream=True
    )
